$(document).ready(function() {
    ...
    if(pointAttachments) {
        for(var i = 0; i < pointAttachments.length; i++) {
            var fieldsetAttachment = $('form#weight-and-dimensions > fieldset > fieldset > fieldset').get(i);
            ...
            if(['jpg', 'png', 'gif', 'jpeg'].indexOf(extension) >= 0) {
                var filePath = system.basePath + '/content/weight-and-dimensions/' + pointAttachments[i];
    ...
    if(errorAttachments) {
        var attachmentsFieldset = $('form#weight-and-dimensions > fieldset > fieldset');
        for(var k in errorAttachments) {
            var filename = $(attachmentsFieldset).find('input[name="weight-and-dimensions[' + attachmentsIndex + '][' + k + '][filename]"]');
    ...
    $("#weight-and-dimensions").on("click", "#add-point-btn", function(e) {
        e.preventDefault();
        showAttachmentFieldset();
    });

    $("#weight-and-dimensions").on("click", "#del-point-btn", function(e) {
        e.preventDefault();
    ...
    function showAttachmentFieldset() {
        var template =
            '<fieldset>' +
            '<input type="hidden" name="weight-and-dimensions[' + attachmentsIndex + '][__index__][pointAttachmentId]" value>' +
            '<div class="control-group">' +
            '<label class="control-label required">' + system.translations.Title + '</label>' +
            '<div class="controls"><input name="weight-and-dimensions[' + attachmentsIndex + '][__index__][title]" required="required" value="" type="text"></div>' +
            '</div>' +
            '<div class="control-group">' +
            '<label class="control-label required" for="attachment_id">' + system.translations.File + '</label>' +
            '<div class="controls">' +
            '<div class="fileupload fileupload-new" data-provides="fileupload">' +
            '<input type="hidden" value="" name="weight-and-dimensions[' + attachmentsIndex + '][__index__][filename]">' +
            ...
            '<input type="file" name="weight-and-dimensions[' + attachmentsIndex + '][__index__][filename]">' +
            '</span>' +
            ...
            '<div class="control-group">' +
            '<div class="controls">' +
            '<input type="submit" name="weight-and-dimensions[' + attachmentsIndex + '][0][delete]" id="del-point-btn" class="btn btn-primary btn-default" value="' + system.translations.deleteAttachment + '">' +
            '</div>' +
            '</div>' +
            '</fieldset>';

        var currentCount = $('form#weight-and-dimensions > fieldset > fieldset > fieldset').length;
        template = template.replace(/__index__/g, currentCount);

        $('form#weight-and-dimensions > fieldset > fieldset').append(template);
        $('form#weight-and-dimensions > fieldset > fieldset > fieldset:last .fileupload').fileupload();
